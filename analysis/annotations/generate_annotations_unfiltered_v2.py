...
- Marker names with supergame, round, and market period counters
...
- Supergame numbers from 'chat_noavg', 'chat_noavg2', 'chat_noavg3', 'chat_noavg4'
- Round numbers start at 1 and increment ONLY when page_name == 'Results'
- Market period numbers start at 1 per round and increment ONLY when page_name == 'MarketPeriod' (reset to 1 on new round)
"""
...
DEFAULT_OUTPUT = str((SCRIPT_DIR / "annotation_generated_unfiltered_v2.csv").resolve())
...
def extract_supergame_number(app_name):
    ...
    Args:
        app_name: app name like 'chat_noavg', 'chat_noavg2', 'chat_noavg3', 'chat_noavg4'
    ...
    # Match 'chat_noavg' without number (supergame 1)
    if app_lower == 'chat_noavg':
        return 1
    
    # Match 'chat_noavg2', 'chat_noavg3', 'chat_noavg4'
    match = re.search(r'chat_noavg(\d+)', app_lower)
    if match:
...
def build_marker_names(df):
    """Build marker names with supergame, round, and market period numbering.
    ...
    Returns:
        ...
        - non-supergame pages: just page_name
        - supergame pages: s{N}r{R}m{M}page_name
    
    Numbering rules (exact matching):
    - Round number (R) starts at 1 and increments AFTER seeing page_name == 'Results'
    - Market period (m) starts at 1 each round and increments ON the page_name == 'MarketPeriod' row
    - 'ResultsWaitPage' and 'MarketPeriodWait' must NOT affect counters
    """
    df = df.copy()
    ...
    for participant_id in df['participant_id_in_session'].unique():
        ...
        current_supergame = None
        round_counter = 1  # Start at 1 when entering a supergame
        market_period_counter = 1  # Starts at 1 within each round
        round_nums = []
        market_period_nums = []
        
        for idx, row in participant_df.iterrows():
            sg = row['supergame_num']
            page_name = str(row['page_name'])
            
            if pd.notna(sg):
                # Entering or switching supergames resets counters
                if current_supergame != sg:
                    current_supergame = sg
                    round_counter = 1
                    market_period_counter = 1
                
                if page_name == 'Results':
                    # Record current counters BEFORE incrementing (Results shows old round)
                    round_nums.append(round_counter)
                    market_period_nums.append(market_period_counter)
                    # Now increment for next rows
                    round_counter += 1
                    market_period_counter = 1
                elif page_name == 'MarketPeriod':
                    # Increment market period BEFORE recording (MarketPeriod shows new period)
                    market_period_counter += 1
                    round_nums.append(round_counter)
                    market_period_nums.append(market_period_counter)
                else:
                    # Other pages just record current counters
                    round_nums.append(round_counter)
                    market_period_nums.append(market_period_counter)
            else:
                # Non-supergame apps
                round_nums.append(0)
                market_period_nums.append(0)
        
        df.loc[participant_mask, 'round_num'] = round_nums
        df.loc[participant_mask, 'market_period_num'] = market_period_nums
    
    # Build marker names based on app type
    def create_marker_name(row):
        page_name = row['page_name']
        supergame = row['supergame_num']
        round_num = row['round_num']
        market_period = row['market_period_num']
        
        # Non-supergame apps - no prefix
        if pd.isna(supergame):
            return page_name
        
        # Supergame pages - include s, r, and m numbering
        return f"s{int(supergame)}r{int(round_num)}m{int(market_period)}{page_name}"
    
    df['marker_name'] = df.apply(create_marker_name, axis=1)
    
    # Clean up temporary columns
    df = df.drop(columns=['supergame_num', 'round_num', 'market_period_num'])
