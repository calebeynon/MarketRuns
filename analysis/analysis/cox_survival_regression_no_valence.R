# Purpose: Cox proportional hazards survival regression — no valence
#          8 discrete emotions only, valence excluded.
# Author: Claude Code
# Date: 2026-03-09
...
OUTPUT_PATH <- "analysis/output/tables/cox_survival_regression_no_valence.tex"
...
source("analysis/analysis/cox_survival_panel_a_no_valence.R")
source("analysis/analysis/cox_survival_panel_b_no_valence.R")

# Discrete emotions (no valence)
...
main <- function() {
  ...
  df_em <- df[complete.cases(df[, .SD, .SDcols = c(DISCRETE_EMOTIONS, ALL_TRAITS)])]
...
# =====
# Variable order for flat 4-column table (no valence)
# =====
...
build_preamble <- function() {
  caption <- "Cox survival regression --- no valence (hazard ratios)"
  c("", "\\begingroup", "\\centering", "\\scriptsize",
    "\\setlength{\\LTcapwidth}{\\textwidth}",
    paste0("\\begin{longtable}{l",
           "*{4}{>{\\centering\\arraybackslash}p{2.2cm}}}"),
    sprintf("\\caption{%s} \\label{tab:cox_survival_regression_no_valence} \\\\",
            caption))
}
