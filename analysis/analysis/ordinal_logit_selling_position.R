# Purpose: Mixed ordinal logit regressions for selling position (rank 1-4)
#          Model 1: Full sample (non-sellers = rank 4)
#          Model 2: Sellers only (did_sell == 1)
# Author: Claude Code
# Date: 2026-02-10
...
main <- function() {
  ...
  cat("\n--- Fitting Model 1: Full Sample ---\n")
  model1 <- fit_clmm(df, "Full Sample")

  df_sellers <- df[did_sell == 1]
  cat("\n=== Sell rank frequencies (sellers only) ===\n")
  print(table(df_sellers$sell_rank))

  cat("\n--- Fitting Model 2: Sellers Only ---\n")
  model2 <- fit_clmm(df_sellers, "Sellers Only")

  build_two_model_table(model1, model2, OUTPUT_PATH)
  cat("\nDone!\n")
}
...
load_and_prepare <- function(file_path) {
  ...
  # Drop rows with missing emotion data
  n_before <- nrow(df)
...
# =====
# Coefficient extraction for clmm
# =====
extract_clmm_coefs <- function(model) {
  s <- summary(model)
  beta_dt <- extract_beta_table(s)
  alpha_dt <- extract_alpha_table(s, model)
...
extract_beta_table <- function(s) {
  ...
  data.table(
    var = rownames(beta), est = beta[, "Estimate"],
    se = beta[, "Std. Error"], pval = beta[, "Pr(>|z|)"]
  )
}

# Extract threshold params; use vcov when Hessian is available
extract_alpha_table <- function(s, model) {
  tnames <- names(model$alpha)
  ests <- unname(model$alpha)
...
build_two_model_table <- function(model1, model2, output_path) {
  ...
  lines <- build_header()
  lines <- append_show_rows(lines, c1$beta, c2$beta)
...
header_column_block <- function() {
  dep_line <- "   Dependent Variable: & \\multicolumn{2}{c}{Selling Position (ordinal)}\\\\"
  c("   \\midrule \\midrule",
    dep_line,
...
append_show_rows <- function(lines, beta1, beta2) {
  for (v in SHOW_VARS) {
    label <- if (v %in% names(VAR_DICT)) VAR_DICT[v] else gsub("_", "\\\\_", v)
    c1 <- format_coef_cell(beta1, v)
    c2 <- format_coef_cell(beta2, v)
    lines <- c(lines,
...
format_threshold_row <- function(tname, coefs1, coefs2) {
  label <- gsub("\\|", "$|$", tname)
  c1 <- format_coef_cell(coefs1, tname)
  c2 <- format_coef_cell(coefs2, tname)
  c(sprintf("   %-25s& %s & %s\\\\", label, c1[1], c2[1]),
    sprintf("   %-25s& %s & %s\\\\", "", c1[2], c2[2]))
}
...
append_threshold_rows <- function(lines, alpha1, alpha2) {
  ...
  all_thresholds <- union(alpha1$var, alpha2$var)
  for (tname in all_thresholds) {
    lines <- c(lines, format_threshold_row(tname, alpha1, alpha2))
  }
  return(lines)
}
...
append_footer <- function(lines) {
  controls_note <- paste0(
    ...
    "segment dummies, round. Player random effects included."
  )
  c(lines,
    "   \\midrule \\midrule",
