...
#          No-valence specification: 8 discrete emotions, valence excluded.
...
OUTPUT_PATH <- "analysis/output/tables/ro_logit_selling_position_no_valence.tex"
...
PREDICTORS_NO_TRAITS <- c(DISCRETE_EMOTIONS, DEMOGRAPHICS)
...
load_and_prepare <- function(file_path) {
  ...
  n_before <- nrow(df)
  df <- df[complete.cases(df[, ..DISCRETE_EMOTIONS])]
...
longtable_header <- function() {
  cap <- paste0("\\caption{Ranked-order logit: selling position ",
                "within group-round (no valence)} ",
                "\\label{tab:ro_logit_selling_position_no_valence} \\\\")
...
longtable_body <- function(coefs) {
  lines <- c()
  lines <- add_section(lines, DISCRETE_EMOTIONS, coefs, "all")
