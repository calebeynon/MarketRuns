# Purpose: Panel A (all sellers) Cox survival regression — no-traits/with-traits split
# Author: Claude Code
# Date: 2026-02-24
...
# =====
# Model 1: No traits RE Cox (emotions + valence, no personality traits)
# =====
run_cox_panel_a_no_traits <- function(df) {
  cat("[Panel A M1] No traits RE Cox (coxme)...\n")
...
# =====
# Model 2: With traits RE Cox (emotions + valence + personality traits)
# =====
run_cox_panel_a_with_traits <- function(df) {
  cat("[Panel A M2] With traits RE Cox (coxme)...\n")
  f <- Surv(period_start, period, sold) ~ dummy_1_cum + dummy_2_cum + dummy_3_cum +
    int_1_1 + int_2_1 + int_2_2 + int_3_1 + int_3_2 + int_3_3 +
    fear_mean + anger_mean + contempt_mean + disgust_mean +
    joy_mean + sadness_mean + surprise_mean + engagement_mean +
