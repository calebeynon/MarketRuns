...
#          2 models, both full-trait spec: Sellers Only | All Participants.
# Author: Claude Code  |  Date: 2026-05-20
...
OUTPUT_PATH <- "analysis/output/tables/ro_logit_two_column.tex"
...
main <- function() {
  ...
  m_sellers <- fit_cox(df_sellers, PREDICTORS_WITH_TRAITS, "Sellers Only")
  m_all <- fit_cox(df, PREDICTORS_WITH_TRAITS, "All Participants")

  coefs <- lapply(list(m_sellers, m_all), extract_coefs)
  n_str <- c(uniqueN(df_sellers$stratum), uniqueN(df$stratum))
  fits <- mapply(extract_fit, list(m_sellers, m_all), n_str, SIMPLIFY = FALSE)
...
longtable_header <- function() {
  cap <- paste0("\\caption{Ranked-order logit: selling position ",
                "within group-round} \\label{tab:ro_logit_two_column} \\\\")
  hdr <- c("", "\\begingroup", "\\centering", "\\scriptsize",
    "\\setlength{\\LTcapwidth}{\\textwidth}",
    "\\begin{longtable}{l*{2}{>{\\centering\\arraybackslash}p{2.6cm}}}", cap)
...
longtable_body <- function(coefs) {
  lines <- c()
  lines <- add_section(lines, EMOTIONS_AND_VALENCE, coefs)
  lines <- add_section(lines, DEMOGRAPHICS, coefs)
  lines <- add_section(lines, PERSONALITY_TRAITS, coefs)
  lines
}

add_section <- function(lines, vars, coefs) {
  for (v in vars) lines <- c(lines, format_var_row(v, coefs))
  lines
}

format_var_row <- function(varname, coefs) {
  cells <- lapply(coefs, format_cell, varname)
  vals <- build_row_values(cells)
  label <- get_var_label(varname)
  c(sprintf("   %-25s& %s & %s \\\\", label, vals$est[1], vals$est[2]),
    sprintf("   %-25s& %s & %s \\\\", "", vals$se[1], vals$se[2]))
}

build_row_values <- function(cells) {
  est <- sapply(cells, `[`, 1)
  se <- sapply(cells, `[`, 2)
  list(est = est, se = se)
}
...
longtable_fit <- function(fits) {
  ...
  fmt_row <- function(label, vals) {
    sprintf("   %-25s& %s & %s \\\\", label, vals[1], vals[2])
  }
  c("   \\midrule",
    "   \\emph{Fit statistics} & & \\\\",
...
longtable_footer <- function() {
  note <- function(txt) {
    sprintf("   \\multicolumn{3}{l}{\\emph{%s}} \\\\", txt)
  }
  c("   \\midrule \\midrule",
    note("Coefficients are log-hazard ratios (positive = earlier selling), standardized (z-scored)."),
    note("Both columns use the full specification (emotions, demographics, personality traits)."),
