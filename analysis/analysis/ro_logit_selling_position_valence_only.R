...
#          valence only. 4 models (Full/Sellers x No Traits/With Traits).
# Author: Claude Code  |  Date: 2026-03-09
...
OUTPUT_PATH <- "analysis/output/tables/ro_logit_selling_position_valence_only.tex"
...
PREDICTORS_NO_TRAITS <- c("valence_p95", DEMOGRAPHICS)
...
main <- function() {
  ...
  print_fit_comparison(list(m1, m2, m3, m4))
...
load_and_prepare <- function(file_path) {
  ...
  n_before <- nrow(df)
  df <- df[complete.cases(df[, .(valence_p95)])]
  cat("Dropped", n_before - nrow(df), "rows with missing valence\n")
...
# =====  Fit comparison (console output)
print_fit_comparison <- function(models) {
  labels <- c("Full-NoTraits", "Full-WithTraits",
              "Sellers-NoTraits", "Sellers-WithTraits")
  cat("\n====== Fit Comparison ======\n")
  for (i in seq_along(models)) {
    s <- summary(models[[i]])
    cat(sprintf("  %-20s  Log-lik: %8.1f  Concordance: %.3f\n",
                labels[i], models[[i]]$loglik[2], s$concordance["C"]))
  }
  cat("============================\n")
}
...
longtable_header <- function() {
  cap <- paste0("\\caption{Ranked-order logit: selling position ",
                "within group-round (valence only)} ",
                "\\label{tab:ro_logit_selling_position_valence_only} \\\\")
...
longtable_body <- function(coefs) {
  lines <- c()
  lines <- add_section(lines, c("valence_p95"), coefs, "all")
...
build_row_values <- function(cells, type) {
  est <- sapply(cells, `[`, 1)
  se <- sapply(cells, `[`, 2)
  # Personality trait rows blank in no-traits columns (1 and 3)
  if (type == "traits") {
    est[c(1, 3)] <- ""
    se[c(1, 3)] <- ""
  }
