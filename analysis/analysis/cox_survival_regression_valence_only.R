# Purpose: Cox proportional hazards survival regression — valence only
# Author: Claude Code
# Date: 2026-03-09
...
OUTPUT_PATH <- "analysis/output/tables/cox_survival_regression_valence_only.tex"
...
source("analysis/analysis/cox_survival_panel_a_valence_only.R")
source("analysis/analysis/cox_survival_panel_b_valence_only.R")
...
main <- function() {
  ...
  df_em <- df[complete.cases(df[, .SD, .SDcols = c("valence_mean", ALL_TRAITS)])]
  cat("Base:", nrow(df), "| Valence+trait-complete:", nrow(df_em), "\n")

  panel_a <- run_cox_panel_a(df_em)
  panel_b <- run_cox_panel_b(df_em)

  print_fit_summary(panel_a, panel_b)
...
# =====
# Data preparation (mirrors cox_survival_regression.R)
# =====
...
# =====
# Print log-likelihood and event counts to console
# =====
print_fit_summary <- function(panel_a, panel_b) {
  models <- list(panel_a$no_traits, panel_a$with_traits,
                 panel_b$no_traits, panel_b$with_traits)
  labels <- c("Panel A No Traits", "Panel A With Traits",
              "Panel B No Traits", "Panel B With Traits")
  cat("\n--- Fit Summary (Valence Only) ---\n")
  for (i in seq_along(models)) {
    fit <- extract_cox_fit(models[[i]])
    cat(sprintf("  %s: LL=%.1f, events=%d, n=%d\n",
                labels[i], fit$log_lik, fit$events, fit$n))
  }
  cat("\n")
}

# =====
# Variable order for flat 4-column table (valence only)
# =====
get_var_order <- function() {
  cascade <- c("dummy_1_cum", "dummy_2_cum", "dummy_3_cum")
  c(cascade, INTERACTION_VARS, "signal", "valence_mean",
    COX_CONTROLS, COX_TRAITS)
}

# =====
# 4-column LaTeX coefficient formatting
# =====
...
build_preamble <- function() {
  caption <- "Cox survival regression --- valence only (hazard ratios)"
  c("", "\\begingroup", "\\centering", "\\scriptsize",
    "\\setlength{\\LTcapwidth}{\\textwidth}",
    paste0("\\begin{longtable}{l",
           "*{4}{>{\\centering\\arraybackslash}p{2.2cm}}}"),
    sprintf("\\caption{%s} \\label{tab:cox_survival_regression_valence_only} \\\\",
            caption))
}
