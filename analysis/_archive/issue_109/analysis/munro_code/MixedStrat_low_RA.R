...
a=0.5# CRRA coefficient, must be >0
if(a==1){u<-function(x){log(x)}} else {u<-function(x){x^(1-a)/(1-a)}} #defines CRRA u function
...
V_1=pmax(u(prices[4]),(beliefs*mean(u(prices[4]))+(1-beliefs)*u(v)))
term_value=(1-beliefs)*u(v)+beliefs*mean(u(prices[4]))
...
V_2=pmax(u(prices[3]),(beliefs*mean(u(prices[c(3,4)]))+(1-beliefs)*u(v)))
new_V_2=V_2
...
while(go==0&iter<max_iter){
  ...
  term_value=(1-beliefs)*u(v) + beliefs * mean(u(prices[c(3,4)]))
  
  W_2=l*term_value+(1-l)*cont_value
  
  exp_W=colSums(s_probs*rbind(W_2,W_1))
  
  exp_price=colSums(s_probs*rbind(rep(u(prices[3]),num_beliefs),rep(mean(u(prices[c(3,4)])),num_beliefs)))
...
#initialize value function
V_3=pmax(u(prices[2]),(beliefs*mean(u(prices[c(2,3,4)]))+(1-beliefs)*u(v)))
new_V_3=V_3
#strategies
x1=0.402
x2=1
...
while(go==0&iter<max_iter){
  ...
  term_value=(1-beliefs)*u(v) + beliefs * mean(u(prices[c(2,3,4)]))
  
  W_3=l*term_value+(1-l)*cont_value
  
  exp_W=colSums(s_probs*rbind(W_3,W_2,W_1))
  
  exp_price=colSums(s_probs*rbind(rep(mean(u(prices[c(2)])),num_beliefs),rep(mean(u(prices[c(2,3)])),num_beliefs),rep(mean(u(prices[c(2,3,4)])),num_beliefs)))
...
#initialize value function
V_4=pmax(u(8),(beliefs*mean(u(prices[c(1,2,3,4)]))+(1-beliefs)*u(v)))
new_V_4=V_4
#strategies
x1=0.11
x2=0.989
...
x4=1
sig=(beliefs>=0.68)*1
sig[num_beliefs-7]=x1
sig[num_beliefs-6]=x2
#sig[num_beliefs-5]=x3
#sig[num_beliefs-6]=x4
...
while(go==0&iter<max_iter){
  ...
  term_value=(1-beliefs)*u(v) + beliefs * mean(u(prices[c(1,2,3,4)]))
  
  W_4=l*term_value+(1-l)*cont_value
  
  exp_W=colSums(s_probs*rbind(W_4,W_3,W_2,W_1))
  
  exp_price=colSums(s_probs*rbind(rep(u(prices[c(1)]),num_beliefs),rep(mean(u(prices[c(1,2)])),num_beliefs),rep(mean(u(prices[c(1,2,3)])),num_beliefs),rep(mean(u(prices[c(1,2,3,4)])),num_beliefs)))
