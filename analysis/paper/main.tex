...
\subsection{Implied Risk Aversion}
\label{sec:implied_risk_aversion}

We recover each participant's CRRA risk-aversion parameter $\alpha$ by maximum likelihood over their hold/sell decisions. For each participant we sum the Bernoulli log-likelihood of the observed outcome in every period they still hold the asset, using the equilibrium selling probability $\sigma(n, \pi; \alpha)$ derived in Section~\ref{sec:market model} evaluated at that period's holder count $n$ and group posterior $\pi$. The MLE is computed on a grid $\alpha \in [0, 1]$ with step $0.001$, and 95\% confidence intervals are the likelihood-ratio set $\{\alpha : 2(\mathrm{LL}_{\max} - \mathrm{LL}(\alpha)) \le 3.84\}$.

Figure~\ref{fig:implied_risk_aversion} sorts the 95 participants by their point estimate and plots the MLE with its LR confidence interval, colored by treatment. Most participants are estimated at or near the lower bound of the grid: 52 of 95 sit at $\alpha = 0$, and the median CI width is $0.014$. Participants with positive estimates cluster tightly around $\alpha \approx 0.11$. The upper end of the grid is never reached; the largest point estimate is $0.455$. The two treatments (Random and Average simultaneous-seller payoffs) produce visually indistinguishable distributions of implied $\alpha$.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{implied_risk_aversion.pdf}
  \caption{Per-participant experiment-implied CRRA $\alpha_{\text{MLE}}$ with likelihood-ratio 95\% confidence intervals. Participants are sorted ascending by point estimate. Color indicates treatment.}
  \label{fig:implied_risk_aversion}
\end{figure}
