...
We estimate Cox proportional hazards survival models treating the within-round period as the survival time axis and selling as the failure event. Table~\ref{tab:cox_survival_regression} reports hazard ratios from random-effects Cox models (coxme) for two samples---All Sellers and First Sellers---each estimated with and without personality traits. All specifications include eight discrete emotions (fear, anger, contempt, disgust, joy, sadness, surprise, engagement) and a composite valence measure. Columns~(1) and~(3) present the base specification; columns~(2) and~(4) add personality traits (state anxiety, impulsivity, risk tolerance, and the Big Five: extraversion, agreeableness, neuroticism, openness, conscientiousness) as additional covariates.

\clearpage

\input{cox_survival_regression}

Regarding detected emotions, ww find that Valence decreases the likelihood of selling while Joy increases it. Both effects are significant at the 1\% level.  However, iMotions may have misconstrued smiles during negative emotions as signals of Joy \citep{forman2020capturing}. \cite{hoque2012exploring} find that people also smile in frustration. 
