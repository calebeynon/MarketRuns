...
We parameterize preferences with the power utility $u = w^{\theta}$, where $\theta$ is risk tolerance (consistent with our trait measure); this is the CRRA family with coefficient $\alpha = 1 - \theta$, so $\theta = 1$ is risk neutral and $\theta < 1$ is risk averse. Table~\ref{tab:equilibrium_tau_cutoffs} reports the equilibrium run-region cutoffs for Treatment~1 (Random) and Treatment~2 (Average) at $\theta = 1$ and $\theta = 0.5$, derived from the equilibrium selling strategy $\sigma(n, \pi)$. We report $\overline{\tau}$, the highest belief at which selling begins ($\sigma > 0$), and $\underline{\tau}$, the highest belief at which selling is certain ($\sigma = 1$); a run occurs whenever $\pi < \overline{\tau}$. Beliefs are expressed as $\pi = \Pr(z = G)$, the mirror image of the $\Pr(z = B)$ convention in \cite{magnani2020dynamic}.

At $\theta = 1$ (risk neutrality) the two treatments coincide, as expected, since the average and random payoff schemes share the same mean. Under risk aversion ($\theta = 0.5$) the onset cutoff $\overline{\tau}$ remains identical across treatments, while the certain-run cutoff $\underline{\tau}$ is weakly higher in the Average treatment for the second and third sellers. The Average scheme thus does not begin runs at less pessimistic beliefs than the Random scheme; if anything it reaches the certain-run regime at slightly more optimistic beliefs, deeper into the cascade.

\begin{table}[H]
  \centering
  \caption{Equilibrium run-region cutoffs $\overline{\tau}$ and $\underline{\tau}$ by seller position and risk tolerance $\theta$, under the Random (Treatment~1) and Average (Treatment~2) payoff schemes. $\overline{\tau}$ marks the onset of selling ($\sigma > 0$) and $\underline{\tau}$ the onset of certain selling ($\sigma = 1$); a run occurs for $\pi = \Pr(z = G) < \overline{\tau}$.}
  \label{tab:equilibrium_tau_cutoffs}
  \input{equilibrium_tau_cutoffs}
\end{table}
...
Table~\ref{tab:tobit_n_sellers} reports a Tobit regression of the number of sellers per group-round. Model~(1) regresses the seller count on treatment, segment, and round; Model~(2) adds the bad-state indicator. Detailed interpretation appears in the group-round-level analysis below.

% Table 4: Tobit regression, number of sellers per group-round
\begin{table}[H]
  \centering
...
Table~\ref{tab:tobit_n_sellers_interactions} extends this specification with Treatment~2~$\times$~Segment interaction terms, allowing the treatment effect on the number of sellers to vary across segments. Model~(1) controls for round, treatment, and segment; Model~(2) adds the bad-state indicator. The interaction with Segment~3 is marginally significant, indicating a stronger Treatment~2 effect in that chat-available segment.

% Table 4b: Tobit regression with Treatment 2 x Segment interactions
\begin{table}[H]
  \centering
  \caption{Tobit regression: number of sellers per group-round, Treatment~2~$\times$~Segment interactions}
  \label{tab:tobit_n_sellers_interactions}
  \input{tobit_n_sellers_interactions}
\end{table}

\subsubsection{First Sellers}

Table~\ref{tab:ols_first_sale_behavior} reports pooled OLS regressions characterizing each group-round's first sale. Models~(1)--(2) use the seller's belief at the time of first sale as the outcome; Models~(3)--(4) use the period of the first sale. The even-numbered models add the bad-state indicator to the treatment, segment, and round controls.

Table~\ref{tab:cox_selling_four_column} reports Cox survival regressions of selling hazard across four risk sets. Model~(1) restricts attention to first sellers, Model~(2) to reactive (rushed second) sellers, Model~(3) to all sellers, and Model~(4) to all participants including non-sellers. Each model reports hazard ratios with the full set of emotion, demographic, and personality-trait covariates.

\subsubsection{Rushed Second Sellers}

Rushed second sellers are analyzed in Model~(2) of Table~\ref{tab:cox_selling_four_column}, which restricts the risk set to person-periods that immediately follow a group-mate's sale.

\subsubsection{All Sellers}

All-seller and full-sample selling behavior are summarized in Models~(3) and~(4) of Table~\ref{tab:cox_selling_four_column}.

Table~\ref{tab:ro_logit_two_column} reports rank-ordered logit regressions of selling position within the group-round. Model~(1) uses the sellers-only sample; Model~(2) uses the full sample of sellers and non-sellers, with all emotion, demographic, and personality-trait covariates.

% Table 5: pooled OLS first-sale behavior (tabular without internal caption)
\begin{table}[H]
  \centering
  \caption{Pooled OLS: belief and period at first sale}
  \label{tab:ols_first_sale_behavior}
  \input{ols_first_sale_behavior}
\end{table}

% Table 6: four-column Cox survival (longtable with built-in caption/label)
\input{cox_selling_four_column}

% Table 7: two-column rank-ordered logit (longtable with built-in caption/label)
\input{ro_logit_two_column}
...
Table \ref{tab:ro_logit_selling_position} presents results from several rank-ordered logit specifications with the hazard rate for selling within the group-round as the outcome variable. Equation \ref{eq:ro_logit} presents the specification.
  ...
  \begin{split}                                                                                                                                                                       
      ...
      & \boldsymbol{\beta}_{\text{emo}}^{\prime} \mathbf{Emotion}_i + \boldsymbol{\beta}_{\text{demo}}^{\prime} \mathbf{Demo}_i  + \boldsymbol{\beta}_{\text{trait}}^{\prime} \mathbf{Trait}_i \bigr)
  \end{split}
  \label{eq:ro_logit}
  \end{equation}
...
Although Table \ref{tab:tobit_n_sellers} presents the effect of each segment on the expected number of sellers in a market, there are two possible channels from which this downward effect on selling originates. Participants could learn more about the experiment as it progresses (even in the absence of communication), or there could be some effect from communication. A limitation of our within-subjects design is that communication always follows the no-communication segments, so any non-communicative learning effects would bias our estimate of a communication effect. We estimate the following regression,
\begin{equation}
  \begin{split}
n\_sellers_{gr} = \beta_1 \cdot \text{BadState}_r &+ \lambda \cdot s_g + \tau \cdot \mathbf{1}[s_g \geq 3] + \delta \cdot \text{Treatment2}_g \times \mathbf{1}[s_g \geq 3] \\+
  &\alpha_{\text{session}} + \gamma_{\text{round}} + \varepsilon_{gr}
    \end{split}
  \label{eq:comms}
\end{equation}
where $s_g \in {1, 2, 3, 4}$ is the segment index for group-round $gr$, $\lambda$ is the per-segment learning rate, and $\tau$ is the communication effect.

We present an additional assumption that allows for the identification of communication effects. Suppose that the learning rate observed in pre-chat segments (1 and 2) would have continued at the same rate into chat segments, absent the chat intervention. Then $\tau$ captures the additional change in sellers per market beyond the no-communication learning effect. Formally, the assumption

\begin{equation}
  \begin{split}
E[n\_sellers_{g,s,r}^{(0)} \mid \mathbf{X}_{gr}] = \alpha + \lambda \cdot s \quad \forall \ s \in \{2,3,4\}
  \end{split}
  \end{equation}
  \begin{equation}
  \begin{split}
      \implies \tau &= \underbrace{E[\Delta n\_sellers_{g,3,r} \mid \mathbf{X}_{gr}] - E[\Delta n\_sellers_{g,2,r} \mid \mathbf{X}_{gr}]}_{\text{observable}} \\
      &= \underbrace{E[n\_sellers_{g,s,r}^{(1)} - n\_sellers_{g,s,r}^{(0)} \mid s \geq 3, \mathbf{X}_{gr}]}_{\text{communication effect}}
  \end{split}
  \end{equation}
where $\Delta n\_sellers_{g,s,r} \equiv E[n\_sellers_{g,s,r} \mid \mathbf{X}_{gr}] - E[n\_sellers_{g,s-1,r} \mid \mathbf{X}_{gr}]$ is the cross-segment change in sellers for group $g$ in round $r$. $n\_sellers_{g,s,r}^{(0)}$ is the potential number of sellers for group $g$ in segment $s$ in round $r$ absent communication, and $\mathbf{X}_{gr} = (\text{BadState}_r, \text{Treatment}_g, \text{Session}_g, \text{Round}_r)$.


It is worth noting that $\tau$ identifies the communication effect on selling compared to the unobserved potential outcome of not being able to communicate in the same group at the same time. This does not identify the effect of market participants chatting with each other about the rules of the experiment separately from a general communicative/cooperative effect. When we refer to the ``communication'' effect, it refers to the effect of \textit{all} participant chat, not just chat about strategy or coordination. 

It may be unreasonable to assume non-communicative learning trends to be linear. The identification would still hold if those learning trends are concave, meaning the sellers per segment diminishes over time. In this case, $\tau$ would be biased \textit{towards} 0. Our effects found to be negative and significant would underestimate the communication effect, but still be significant. Our identification would fail in the convex case where learning accelerates through segments.  


Table \ref{tab:did_learning_communication} presents results from the regression specified above. We include a treatment effect as well to test whether the communication effect is heterogeneous. Consistent with previous results, the bad state is associated with a significant increase in the number of sellers in a market. The communication effect is negative and marginally significant. This means that the number of sellers in a market decreases by 0.4863 when participants are permitted to chat with each other. We also include a test that provides support for our identifying assumption. We test whether the change in sellers, conditional on covariates, differs between the shift from segment 1 to 2 and segments 3 to 4. In both of these transitions, the ability to chat, or not to chat, remains constant. Since $p= 0.7553$, we fail to reject the hypothesis that they differ. This provides support for the assumption that in the absence of a change in communication status, selling trends remain constant. 

Figure \ref{fig:did_segment_effects} displays the learning trend and communication effect visually. Notice that the dashed lines represent the counterfactual outcome in the absence of the chat intervention. There are two of these, one counterfactual projects the 1→2 slope forward into segments 3–4, and the other projects the 3→4 slope backward into segments 1-2. The blue and red arrows represent the size of each of these effects, respectively.
...
\begin{figure}[H]
  ...
  \caption{Segment effects on number of sellers. Points show estimated segment coefficients from Model~(3) in Table~\ref{tab:did_learning_communication} with 95\% standard error clustered confidence intervals. Segment~1 is the reference category.}
  \label{fig:did_segment_effects}
\end{figure}

\subsubsection{Player-period-level regression analysis}

We estimate Cox proportional hazards survival models treating the within-round period as the survival time axis and selling as the failure event. Table~\ref{tab:cox_normal_vs_reactive} reports hazard ratios from two Cox models with cluster-robust standard errors, clustered at the session $\times$ segment $\times$ group level (96 clusters). Column~(1) (\emph{Normal}) uses the full emotions+traits sample with any sale as the event ($N=659$); period-average emotion covariates, cascade dummies, and cumulative-by-previous-period interactions are all included. Column~(2) (\emph{Reactive}) addresses the concern that period-level emotions are contemporaneous with the click: we restrict the risk set to the $N=1{,}082$ person-periods that immediately follow a group-mate's sale, so every row in the fit is mechanically at risk of a cascade-triggered (``reactive'') sale. Within this restricted panel there are 101 sales, which we model as the event; for sellers we replace the period-wide emotion average with the average over the 500 milliseconds immediately preceding the sell click. Cascade and cumulative-by-previous-period interaction terms are omitted from Column~(2) to keep the specification parallel with how a reactive sale is conceptualized: every row in the restricted risk set already follows at least one group-mate's sale by construction. Both specifications include the eight discrete emotions (fear, anger, contempt, disgust, joy, sadness, surprise, engagement), a composite valence measure, signal, round, segment, treatment, age, and gender.
  ...
  \begin{split}                                                                                                                                                                       
     ...
     + \boldsymbol{\gamma}^{\prime} \mathbf{Cascade}_{it} + \beta_s \, \text{Signal}_{it} \\
      & + \boldsymbol{\beta}_{\text{emo}}^{\prime} \mathbf{Emotion}_{it} + \boldsymbol{\beta}_{\text{trait}}^{\prime} \mathbf{Trait}_i  + \boldsymbol{\beta}_{\text{ctrl}}^{\prime} \mathbf{Controls}_{it} + u_i \bigr)
  \end{split}
  \label{eq:coxph}
  \end{equation}
...
\input{cox_survival_normal_vs_reactive}
...
Table \ref{tab:welfare_regression} reports OLS estimates of group-round welfare on individual traits and controls, restricted to state=1 rounds where welfare varies. Welfare is defined as the proportion of the theoretical maximum surplus captured by the group in a given round. Standard errors are clustered at the group level (session $\times$ segment $\times$ group).

\begin{table}[H]
  \centering
  \caption{Welfare regression on individual traits}
  \label{tab:welfare_regression}
  \input{welfare_regression}
\end{table}
...
\section*{Appendix A. Group-round with Emotions and Traits}
\input{ro_logit_selling_position}

\section*{Appendix G. Valence-Only Emotion Specifications}

Table~\ref{tab:ro_logit_selling_position_valence_only} replicates the rank-ordered logit analysis from Table~\ref{tab:ro_logit_selling_position} using only Valence as the emotion variable, omitting the eight discrete emotions. Table~\ref{tab:cox_survival_regression_valence_only} does the same for the Cox survival regression from Table~\ref{tab:cox_normal_vs_reactive}.
...
\section*{Appendix I. Pairwise Correlations}
...
\section*{Appendix. Equilibrium Predictions} \label{sec:app_equilibrium}

\begin{table}[h]
  \centering
  \caption{Average equilibrium belief $\pi = \Pr(z=G)$ at sale by seller position ($k$) and risk aversion ($\alpha$), under the Random and Average payoff schemes. Values averaged over 10{,}000 simulated games using derived equilibrium strategies.}
  \label{tab:equilibrium_thresholds}
  \input{equilibrium_thresholds}
\end{table}
