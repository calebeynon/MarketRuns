...
Table~\ref{tab:summary_demographics} summarizes the subject pool. We recruited 95 subjects across 6 sessions (3 per treatment), with balanced demographics across treatments. Table~\ref{tab:summary_traits} reports personality trait measures by treatment; only conscientiousness differs significantly between treatments ($p<0.05$), while the remaining traits are well balanced. Table~\ref{tab:summary_market_outcomes} compares market outcomes across the no-chat (Segments 1--2) and chat (Segments 3--4) conditions. Selling rates decline from 29.2\% to 17.8\% when chat is available, while average sell period increases, suggesting that communication delays selling decisions.
