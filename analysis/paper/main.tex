...
As a robustness check, we estimate Cox proportional hazards survival models with the same set of covariates as the logit regressions, treating the within-round period as the survival time axis and selling as the event. Table~\ref{tab:cox_survival_regression} in Appendix~B.2 reports hazard ratios from random-effects Cox models for all participants (Panel~A) and first sellers only (Panel~B). The results are qualitatively consistent with the logit specification reported in Appendix~B.1.
...
\subsection*{B.2 Cox Survival Specification}

The following table replicates the selling regression analysis using Cox proportional hazards models. The within-round period serves as the survival time axis, with selling as the failure event. All models include participant-level random intercepts estimated via \texttt{coxme}. Coefficients are reported as hazard ratios: values greater than one indicate an increased hazard of selling, and values less than one indicate a decreased hazard.
