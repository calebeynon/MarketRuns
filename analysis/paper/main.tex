...
We estimate Cox proportional hazards survival models treating the within-round period as the survival time axis and selling as the failure event. Table~\ref{tab:cox_survival_regression} reports hazard ratios from random-effects Cox models (coxme) for all sellers and first sellers, separating facial emotions from the composite valence measure. Columns~(1) and~(3) include seven discrete emotions (fear, anger, contempt, disgust, sadness, surprise, engagement), while columns~(2) and~(4) include only valence.
