...
Table~\ref{tab:tobit_n_sellers_interactions} reports a Tobit regression of the number of sellers per group-round, including Treatment~2~$\times$~Segment interaction terms that allow the treatment effect on the number of sellers to vary across segments. Model~(1) regresses the seller count on round, treatment, segment, and the treatment-segment interactions; Model~(2) adds the bad-state indicator. In Model~(2), the Treatment~2~$\times$~Segment~3 interaction is the only one of the three interactions to reach (marginal) significance ($p<0.10$); however, a joint Wald test of all three interactions fails to reject the null that they are zero ($\chi^2_3 = 4.23$, $p = 0.24$). The pattern is therefore only suggestive of a stronger Treatment~2 effect in the chat segments, not decisive. Because treatment is assigned at the session level (six sessions), cluster-robust standard errors at the session~$\times$~segment~$\times$~group level may understate uncertainty on the treatment and interaction terms, and this small-cluster limitation should be borne in mind when interpreting the interaction estimates. Detailed interpretation appears in the group-round-level analysis below.
