...
We find that markets in treatment 2, the average liquidation treatment, have more sellers. We also find significant effects for the liquidation state and experience.

We estimate the following models:

\begin{equation}
\begin{split}
n\_sellers^{*}_{gr} ={}& \underbrace{\beta_0 + \beta_1 \text{BadState}_{r} + \beta_2 \text{Treatment2}_{g} + \delta \text{Round}_{r} + \sum_{s=2}^{4} \gamma_s \mathbf{1}[\text{Segment}=s]}_{\text{Models (1) and (2)}} \\
&{} + \underbrace{\sum_{s=2}^{4} \zeta_s \text{Treatment2}_{g} \mathbf{1}[\text{Segment}=s]}_{\text{Model (2) only}} + \varepsilon_{gr} \\
n\_sellers_{gr} ={}& \min(\max(n\_sellers^{*}_{gr},0),4)
\end{split}
\label{eq:n_sellers_tobit}
\end{equation}


Table~\ref{tab:tobit_n_sellers_interactions} reports the results from the Tobit regression in equation \ref{eq:n_sellers_tobit}.
In column 1, we find that markets in treatment 2, the average liquidation treatment, have 0.4952 more sellers in expectation than in the random liquidation treatment.
This indicates that participants are more likely to sell when their liquidation payoffs are pooled as an average instead of randomly assigned. This holds even after controlling for round, segment, and state effects.
In both models, we find that markets in the bad state have an average of 1 more seller than in markets in the good state. This is expected as 
bad state markets have more bad signals which pushes investors towards selling. 

Both models indicate that experience decreases the number of sellers in a market. Each additional round within a segment is associated with a 0.07 decrease in sellers. 
Segment 3 and segment 4 both have significant negative effects on the number of sellers, -1.07 and -1.28, respectively. These segments also introduced communication,
so experience vs communication effects will be identified and reported in section \ref{sub:did_learning_communication}.

In model 2, we include a cross effect between treatment and segment. We find a marginally significant effect on 
the treatment 2 x segment 3 coefficient. This indicates that the communication intervention does not decrease 
sellers in markets in the average liquidation treatment. The communication effect is dominated by the markets in the 
random liquidation treatment. This could be explained by a mutual sense of interdependency being stronger initially
in the average liquidation treatment, whereas investors in the random liquidation treatment don't understand the selling externality
until communication is introduced. 

% Table: Tobit regression with Treatment 2 x Segment interactions
\begin{table}[H]
  \centering
...
Table \ref{tab:tobit_n_sellers_interactions} reports results from a Tobit regression specification with number of sellers in a group-round as the output variable. Equation \ref{eq:tobit} is the regression specification with $n\_sellers^*_{gr}$ is the censored number of sellers at 0 and 4.
  \begin{equation}                                                                                                                                                              
  \begin{split}                                                                               
...
Table~\ref{tab:ro_logit_selling_position_valence_only} replicates the rank-ordered logit analysis from Table~\ref{tab:ro_logit_selling_position} using only Valence as the emotion variable, omitting the eight discrete emotions. Table~\ref{tab:cox_survival_regression_valence_only} does the same for the Cox survival regression from Table~\ref{tab:cox_selling_four_column}.
