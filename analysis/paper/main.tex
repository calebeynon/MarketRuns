...
Table~\ref{tab:tobit_n_sellers_interactions} reports a Tobit regression of the number of sellers per group-round, including Treatment~2~$\times$~Segment interaction terms that allow the treatment effect on the number of sellers to vary across segments. Model~(1) regresses the seller count on round, treatment, segment, and the treatment-segment interactions; Model~(2) adds the bad-state indicator. The interaction with Segment~3 is marginally significant, indicating a stronger Treatment~2 effect in that chat-available segment. Detailed interpretation appears in the group-round-level analysis below.
