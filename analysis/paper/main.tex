...
\begin{figure}[H]
  ...
  \caption{Segment effects on number of sellers. Points show estimated segment coefficients from Model~(3) in Table~\ref{tab:tobit_n_sellers} with 95\% standard error clustered confidence intervals. Segment~1 is the reference category.} 
...
\subsection{Market Runs}

We now turn to predicting market run incidence directly. We define a $(w,k)$-run in a group-round as the event that at least $k$ distinct participants liquidate their asset within some $(w+1)$-period window during the round. Sweeping $k \in \{2,3,4\}$ (Panels A, B, C) and $w \in \{0,1,2,3\}$ (columns 1--4 within each estimator) lets us trace how trait and state predictors operate across both the intensity of selling (how many of the four players sell) and its concentration in time (how tightly clustered the sales are). Smaller $w$ isolates panic-like, near-simultaneous selling, while larger $w$ captures more diffuse runs that unfold over multiple periods.

The key identification challenge is that any deviation-from-equilibrium regressor measured across all four players is mechanically simultaneous with the run outcome: the same sales that constitute the run also determine the realized $\pi$ values used to construct the deviation. We address this by restricting the regressor of interest to the \emph{first seller} in each group-round. Because a $(w,k)$-run with $k \geq 2$ requires at least two distinct sellers, the first sale alone never satisfies the run condition; the first seller's behavior is therefore predetermined relative to whether the cascade subsequently \emph{becomes} a run. This breaks the simultaneity between the left-hand side and the deviation regressor while preserving an economically meaningful test of whether early panic propagates into a run.

The regressor of interest is \texttt{dev\_from\_threshold\_first}, defined as $\pi_{\text{at\_sale}}^{\text{first}} - \pi_{\text{threshold}}^{\text{eq}}(\alpha_{\text{first}}, n=4)$, where $\pi_{\text{threshold}}^{\text{eq}}$ is the equilibrium selling threshold evaluated at the first seller's individual risk-aversion parameter $\alpha_{\text{first}}$ and at $n=4$ holders (since the first seller acts before any other liquidations). A negative deviation indicates that the first seller liquidated below the theoretical threshold---i.e., they panicked early. Controls include \texttt{first\_seller\_signal\_correct} (whether the first seller's most recent signal matched the true state), \texttt{first\_sale\_period} (the period of the first sale), $\alpha_{\text{first}}$, group-mean Big Five traits, group-mean impulsivity, group-mean state anxiety, group-mean risk tolerance, \texttt{signal\_correct\_frac} (round-level fraction of correct signals), \texttt{tie\_at\_first\_period} (an indicator for multiple simultaneous sales in the first-sale period), treatment fixed effects, and segment fixed effects. Standard errors are multi-way clustered on \texttt{session\_id} and \texttt{group\_id}, which replaces the prior group-round clustering and accounts for within-session and within-group dependence across rounds. The estimation sample is the set of group-rounds with at least one seller. For each $(w,k)$ combination we estimate both a linear probability model and a logit model; logit columns for which \texttt{feglm} did not converge (a sparse $(w,k)$ cell with separation) are omitted from the table while the corresponding LPM column is retained.

\begin{table}[H]
  \centering
  \caption{Market run regression on first-seller deviation from equilibrium and round-level controls. The unit of observation is the group-round restricted to group-rounds with at least one seller. The regressor of interest is the first seller's $\pi$ at sale minus the equilibrium selling threshold evaluated at the first seller's $\alpha$ and $n=4$ holders. Each panel corresponds to a different threshold $k$ for the number of distinct sellers; columns sweep the window width $w$. Columns (1)--(4) report linear probability model (LPM) estimates; columns (5)--(8) report logit estimates on the same outcomes. Standard errors are multi-way clustered on session and group.}
  \label{tab:market_runs_first_seller_regression}
  \input{market_runs_first_seller_regression}
\end{table}
...
\section*{Appendix J. Welfare Regression}
\label{sec:app_welfare}
