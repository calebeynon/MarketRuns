...
Table~\ref{tab:tobit_n_sellers_interactions} reports a Tobit regression of the number of sellers per group-round, including Treatment~2~$\times$~Segment interaction terms that allow the treatment effect on the number of sellers to vary across segments. Model~(1) regresses the seller count on round, treatment, segment, and the treatment-segment interactions; Model~(2) adds the bad-state indicator. In Model~(2), the Treatment~2~$\times$~Segment~3 interaction is the only one of the three interactions to reach (marginal) significance ($p<0.10$); however, a joint Wald test of all three interactions fails to reject the null that they are zero ($\chi^2_3 = 4.23$, $p = 0.24$).
