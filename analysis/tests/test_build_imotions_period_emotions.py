...
def make_market_df(
    ...
    round_num: int = 1,
    period: int = 1,
    emotion_values: dict = None,
):
    """Create a mock market DataFrame for aggregation testing."""
    data = {
        "segment": [segment] * n_frames,
        "round": [round_num] * n_frames,
        "period": [period] * n_frames,
    }
    # Set emotion columns
    for col in EMOTION_COLS:
        if emotion_values and col in emotion_values:
            data[col] = emotion_values[col]
        else:
            data[col] = [0.5] * n_frames
